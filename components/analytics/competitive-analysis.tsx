...
export function CompetitiveAnalysis() {
  ...
  const exportBookmarks = () => {
    const bookmarkedItems = citations.filter((citation: CitationData) => bookmarkedCitations.has(citation.url))
    if (bookmarkedItems.length === 0) {
      toast.error('No bookmarks to export')
      return
    }

    const exportData = bookmarkedItems.map((item: CitationData) => ({
      title: item.title,
      url: item.url,
      count: item.count,
    ...
    exportToCSV({ bookmarks: exportData } as any, filename)
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={filters.promptCluster} onValueChange={(value: string) => setFilters({...filters, promptCluster: value})}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
        ...
        <CardContent>
          <div ref={citationsTableRef}>
            <Table>
              ...
              <TableBody>
                {citations.map((citation: CitationData, index: number) => (
                  <TableRow key={index}>
                    <TableCell className="font-mono">
                    ...
                    <TableCell>
                      <div className="flex flex-wrap gap-1">
                        {citation.competitors.map((comp: string, idx: number) => (
